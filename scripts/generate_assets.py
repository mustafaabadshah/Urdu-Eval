...
def get_fonts():
    ...
    try:
        ...
        font_urdu = ImageFont.truetype("urdu.ttf", 36)
        ...
        try:
            ...
            font_urdu = font_title
            font_lg = ImageFont.truetype("arialbd.ttf", 20)
            font_badge = ImageFont.truetype("arial.ttf", 13)
        except Exception:
            f = ImageFont.load_default()
            font_sm = font_md = font_bold = font_title = font_urdu = font_lg = font_badge = f
    return font_sm, font_md, font_bold, font_title, font_urdu, font_lg, font_badge


def generate_banner_png(output_path: Path) -> None:
    """Generate high-res 1200x380 banner PNG."""
    width, height = 1200, 380
    im = Image.new("RGB", (width, height), color=(8, 13, 23))
    draw = ImageDraw.Draw(im)
    font_sm, font_md, font_bold, font_title, font_urdu, font_lg, font_badge = get_fonts()
    ...
    draw.text((70, 95), "UE", font=font_title, fill=(52, 211, 153))

    # Titles
    draw.text((150, 85), "UrduEval", font=font_title, fill=(255, 255, 255))
    draw.text((340, 85), "اردو ایویل", font=font_title, fill=(52, 211, 153))
    ...
    for text, dot_color in badges:
        ...
        draw.text((bx + 30, 184), text, font=font_badge, fill=(226, 232, 240))
        bx += bw + 15
    ...
    kpis = [
        ("6+ Core Tasks", "QA, Reasoning, Translation, Summary, MMLU, Roman"),
        ("7+ Metrics", "Exact Match, Token F1, BLEU-4, chrF++, ROUGE-L, Judge"),
        ("0 Fabricated Benchmarks", "Methodologically honest development samples"),
        ("100% Local & Private", "Local-first evaluation without telemetry"),
    ]
    kx = 80
    for title, subtitle in kpis:
        draw.text((kx, 250), title, font=font_bold, fill=(56, 189, 248))
        draw.text((kx, 272), subtitle, font=font_sm, fill=(100, 116, 139))
        kx += 275
...
def generate_architecture_png(output_path: Path) -> None:
    ...
    # Header
    draw.text(
        (30, 30), "UrduEval End-to-End Evaluation Architecture", font=font_lg, fill=(255, 255, 255)
    )
    draw.text(
        (30, 56),
    ...
    for title, subtitle, points, accent_color, x, y, w, h in cards:
        ...
        draw.text((x + 16, y + 16), title, font=font_bold, fill=(248, 250, 252))
        draw.text((x + 16, y + 36), subtitle, font=font_sm, fill=(148, 163, 184))
        py = y + 60
        for pt in points:
            draw.text((x + 16, py), pt, font=font_sm, fill=(226, 232, 240))
            py += 22
...
def generate_report_preview_png(output_path: Path) -> None:
    """Generate high-res 920x520 report preview PNG."""
    ...
    # Header
    draw.text((30, 68), "UrduEval Run Diagnostics Report", font=font_lg, fill=(255, 255, 255))
    draw.text(
        (30, 94),
    ...
    for title, val, ci, color in kpis:
        ...
        draw.text((kx + 16, 135), val, font=font_lg, fill=color)
        draw.text((kx + 16, 160), title, font=font_bold, fill=(226, 232, 240))
        draw.text((kx + 16, 178), ci, font=font_sm, fill=(100, 116, 139))
        kx += 220

    # Filter Toolbar
    draw.rounded_rectangle([30, 215, 340, 250], radius=6, fill=(15, 23, 42), outline=(51, 65, 85))
    draw.text(
        (45, 226), "Search samples by prompt, answer, or ID...", font=font_sm, fill=(100, 116, 139)
    )
    ...
    for name, fbg in filters:
        ...
        draw.text((fx + 16, 226), name, font=font_sm, fill=(255, 255, 255))
        fx += 120
    ...
    draw.text(
        (45, 312),
        "Input Prompt:  پاکستان کا دارالحکومت کون سا شہر ہے؟",
        font=font_md,
        fill=(203, 213, 225),
    )
    draw.text(
        (45, 338),
        "Model Output:  اسلام آباد   (Reference: اسلام آباد)",
        font=font_md,
        fill=(52, 211, 153),
    )
    ...
    draw.text(
        (45, 432),
        "Input Prompt:  اگر احمد کے پاس ۵ سیب ہیں اور وہ ۲ کھا لیتا ہے، تو کتنے باقی ہیں؟",
        font=font_md,
        fill=(203, 213, 225),
    )
    draw.text(
        (45, 458),
        "Model Output:  احمد کے پاس ۲ سیب باقی ہیں   (Expected: ۳ سیب)",
        font=font_md,
        fill=(248, 113, 113),
    )
...
def generate_rich_animated_gif(output_path: Path) -> None:
    ...
    d.text((45, 112), "UrduEval v0.1.1", font=font_bold, fill=text_green)
    d.text(
        (45, 132),
